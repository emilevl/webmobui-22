import { getSongsForArtist, searchSongs, getSong } from '../api'
...
const songLyricsTemplate = songsSection.querySelector('#song-lyrics-item-template');
...
async function renderSong(song, songs) {
  ...
  newSong.querySelector('.lyrics-button').addEventListener('click', async () => {
    const songInfo = await getSong(song.id)
    const newLyrics = songLyricsTemplate.content.cloneNode(true);
    renderLyrics(songInfo);
    newLyrics.querySelector('.list-item-title').innerText = songInfo.title;
    newLyrics.querySelector('.lyrics p').textContent = songInfo.lyrics;
    songsSection.append(newLyrics);
  })
